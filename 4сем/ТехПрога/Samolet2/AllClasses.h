...
namespace Creatures
{
	// RotationalObject - класс, который поворачивает картинку в зависимости от того, куда она направлена, также позвол€ет удобнее еЄ двигать
	...
	// PlanedMovementObject - класс, помогающий передвигать объект по последовательнсти координат (пути). — посто€нной скоростью
	public ref class PlanedMovementObject: RotationalObject
	{
		...
		void move()
		{
			...
			if(Center == Movement_path[progress])
			{
				progress++;
				return;
			}
	...
	// Plane - пассажирский самолЄт. ћаленький и красный.
	public ref class Plane: PlanedMovementObject
	{
		...
		protected: static String ^ ImagePath = "plane.png";
		public: List<Point>^ LandingMovement_path;
		...
		Callback tick()
		{
			...
			if(state == "wait" && isFinished())
			{
				...
				{
					prepare_to_land(LandingMovement_path);
				}
			...
			if(state == "moving_to_hung" && isFinished())
			{
				state = "Idle";
			...
			if(state == "moving_to_hung" && (!isMidPoint() || !loaded))
			{
				move();
				Console::WriteLine("H4");
				return Callback::Default;
			}
			if(state == "moving_to_hung")
			{
				Console::WriteLine("cargo");
				WaitingForCargo();
				return Callback::Bus;
			}
			if((state == "moving_to_VPP" || state == "flying") && isFinished())
			{
				state = "flying";
				Fly_VPP();
				Rotate(Direction::Right);
			...
			if(state == "moving_to_VPP" && (!isMidPoint() || loaded))
			{
				move();
				Console::WriteLine("H6");
				return Callback::Default;
			}
			if(state == "moving_to_VPP")
			{
				Console::WriteLine("cargo");
				WaitingForCargo();
				return Callback::Bus;
	...
	// Bus - автобус, машина дл€ разгрузки пассажирского самолЄта
	...
	// CargoCar - класс грузовой машины, разгружает грузовые самолЄты. Ќаследуетс€ от Bus, может быть горизонтальным и вертикальным
	...
	// CargoPlane - класс грузового самолЄта, он немного больше обычного, а также красный. Ќаследуетс€ от Plane
	public ref class CargoPlane: Plane
	{
		protected: static String ^ ImagePath = "cargoplane.png";
		public: CargoPlane::CargoPlane(Form ^ World, Point location, int size_)
		{
			state = "Idle3";
			size = size_;
			isCargo = true;
