...
private: System::Void button1_Click_1(System::Object^ sender, System::EventArgs^ e) {
	...
	int sink = Convert::ToInt32(comboBox2->SelectedItem->ToString()) - 1;

	const int inf = 1001 * 1001;

	std::vector<std::vector<std::pair<int, int>>> graph_adj_list;
	graph_adj_list.resize(9);
	
	graph_adj_list[0].push_back({ 1, 10 });
	graph_adj_list[0].push_back({ 3, 8 });

	graph_adj_list[1].push_back({ 2, 4 });
	graph_adj_list[1].push_back({ 3, 8 });
	graph_adj_list[1].push_back({ 5, 4 });

	graph_adj_list[2].push_back({ 5, 3 });

	graph_adj_list[3].push_back({ 4, 10 });
	graph_adj_list[3].push_back({ 6, 4 });
	graph_adj_list[3].push_back({ 7, 5 });

	graph_adj_list[4].push_back({ 1, 12 });
	graph_adj_list[4].push_back({ 7, 6 });


	graph_adj_list[5].push_back({ 4, 7 });
	graph_adj_list[5].push_back({ 7, 9 });
	graph_adj_list[5].push_back({ 8, 11 });

	graph_adj_list[6].push_back({ 7, 15 });
	
	graph_adj_list[7].push_back({ 8, 13 });
	...
	auto mini = findMinimalCut(graph_adj_list, src, sink);

	min->Text = mini.first.ToString();
	
	listBox1->Items->Clear();
	for (int i = 0; i < mini.second.size(); ++i) {
		String^ result = (mini.second[i][0]) + " -> " + (mini.second[i][1]) + ": ";
		result += (mini.second[i][2] == inf) ? "∞" : mini.second[i][2].ToString();
